...
var Auth = (function () {
    function Auth(zone, router, session, sqlResource) {
        var _this = this;
        this.router = router;
        this.session = session;
        this.sqlResource = sqlResource;
        // lock: any;
        this.AUTH0_CLIENT_ID = 'pwC5E08ZZFytctumrhmI2bFmakYRGhD2';
        this.AUTH0_DOMAIN = 'ckapilla.auth0.com';
        ...
        this.session = session;
        console.log('Auth constructor');
        this.lock.on('authorization_error', function (auth_error) {
        ...
        this.lock.on('authenticated', function (authResult) {
            ...
            _this.lock.getProfile(authResult.idToken, function (error, profile) {
                ...
                if (_this.isAuthenticated()) {
                    ...
                    _this.parseUserProfile();
                    console.log('have failed route of ' + _this.session.getFailedRoute());
                    if (_this.session.getFailedRoute() > '') {
                        console.log('navigating to failed route: ' + _this.session.getFailedRoute());
                        _this.router.navigate([_this.session.getFailedRoute()]);
                    }
    ...
    Auth.prototype.handleRedirectWithHash = function () {
        var _this = this;
        console.log('in handleRedirectWithHash');
        this.router.events.take(1).subscribe(function (event) {
            ...
            var authResult = _this.auth0.parseHash(window.location.hash);
            if (authResult && authResult.idToken) {
                console.log('manual emit authenticated');
                _this.lock.emit('authenticated', authResult);
            }
            if (authResult && authResult.error) {
                _this.lock.emit('authorization_error', authResult);
            }
            // }
        });
    };
    ...
    Auth.prototype.parseUserProfile = function () {
        if (this.userProfile !== null) {
            this.adminStatus = this.userProfile['adminStatus'];
            ...
            this.mentorStatus = this.userProfile['mentorStatus'];
            ...
            this.studentId = this.userProfile['studentId'];
            ...
            this.session.setUserId(this.userProfile['user_id'].substr('auth0|'.length));
    ...
    Auth.prototype.restoreUserProfile = function () {
        this.userProfile = JSON.parse(localStorage.getItem('profile'));
    ...
    // }
    Auth.prototype.UpdateLastLogin = function (id) {
        console.log('calling SqlResource UpdateLastLogin');
        this.sqlResource.UpdateLastLogin(this.session.userId)
            .subscribe(function (data) { }, function (err) { return console.error('last login Subscribe error: ' + err); }, function () { console.log('lastLogin set'); });
    };
    Auth.prototype.logout = function () {
        ...
        this.zoneImpl.run(function () { return _this.userProfile = undefined; });
